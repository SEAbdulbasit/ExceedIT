...
class SmoothLineChart @JvmOverloads constructor(
    ...
    override fun draw(canvas: Canvas) {
        ...
        for (i in 1 until size) {
            ...
            val d0 = Math.sqrt(
                Math.pow((p.x - p0.x).toDouble(), 2.0) + Math.pow(
                    (p.y - p0.y).toDouble(),
                    2.0
                )
            )
        ...
        if (size > 0) {
            ...
            mPaint.color = Color.WHITE and 0xFFFFFF or 0x10000000
        ...
        for (point in points) {
            canvas.drawCircle(point.x, point.y, mCircleSize / 4, mPaint)
        }
        mPaint.style = Paint.Style.FILL
        mPaint.color = CHART_COLOR
        for (point in points) {
            canvas.drawCircle(point.x, point.y, (mCircleSize - mStrokeSize) / 4, mPaint)
        }
    }
